...
const tableOfContents: TocItem[] = [
  { id: 'baseline-config', title: 'Baseline Configuration' },
  { id: 'parameter-catalog', title: 'Parameter Catalog' },
  { id: 'runtime-adjustments', title: 'Runtime Adjustments' },
  { id: 'verify-retention', title: 'Verify Retention & Memory Usage' },
...
export default function PgStatInsightsConfigurationPage() {
  return (
    ...
    >
          ...
          {gucParameters.map((param) => (
            <div key={param.name} className="border rounded-lg p-4 space-y-2">
              <div>
                ...
                <p className="text-sm">{param.purpose}</p>
              </div>
              <dl className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs uppercase tracking-wide">
                <dt>Default</dt>
                <dd className="text-right normal-case text-sm">{param.defaultValue}</dd>
                <dt>Context</dt>
                <dd className="text-right normal-case text-sm">{param.context}</dd>
              </dl>
              <p className="text-sm">{param.guidance}</p>
            </div>
          ))}
        </div>
      </section>

      <section id="runtime-adjustments">
        <h2>Runtime Adjustments</h2>
        <p>
      ...
      <section id="verify-retention">
        <h2>Verify Retention &amp; Memory Usage</h2>
        <p>
